...
interface MenuListItemsProps {
    properties : MenuListItems;
}

function Menu_List_Items( {properties} : MenuListItemsProps) {
    ...
    const handleClick= (event: React.MouseEvent<HTMLLIElement, MouseEvent>, index: number) => {
    ...
    return (
        properties.menu?.map((item, index) => {
            return <li key={index} onClick={(e) => handleClick(e, index)}>
                <a href="#" 
                    aria-current="page"
                    className={ listIndex.includes(index) ? "inline-flex items-center px-4 py-3 text-white bg-blue-700  active w-full dark:bg-blue-600" : 
                        "inline-flex items-center px-4 py-3  hover:text-gray-900 bg-gray-50 hover:bg-gray-100 w-full dark:bg-gray-800 dark:hover:bg-gray-700 dark:hover:text-white"  }
                >
                {
                    index === 0 ?
